// updateTempLegend: temperature statistics & legend
function updateTempLegend(features) {
    let container = document.getElementById('temp-legend-container');
    if (!container) {
        container = document.createElement('div');
        container.id = 'temp-legend-container';
        ...
        title.textContent = '氣溫統計 & 圖例';

        const icon = document.createElement('span');
        icon.id = 'temp-toggle-icon';
...
function renderTempLegend() {
    ...
    const sorted = [...features]
        .sort((a, b) => isDesc ? b.properties.temperature - a.properties.temperature : a.properties.temperature - b.properties.temperature)
        .slice(0, 20);
    ...
    html += `<div style="margin-bottom: 5px; font-weight: bold; display: flex; justify-content: space-between; align-items: center;">
        <span>氣溫排行 (Top 20)</span>
        <span style="cursor: pointer; padding: 0 5px;" onclick="toggleTempSort()">${sortIcon}</span>
    </div>`;
    ...
    sorted.forEach((f, i) => {
        ...
        html += `<tr style="background: ${rowBg}; cursor: pointer;" onclick="map.flyTo({center: [${coords[0]}, ${coords[1]}], zoom: 12});">
            ...
            <td style="padding: 3px 5px; text-align: right; font-family: monospace;">${p.temperature.toFixed(1)}</td>
        </tr>`;
    });
    html += '</table>';
...
window.toggleTempSort = function() {
    const content = document.getElementById('temp-content');
    if (content) {
...
window.temperatureLayer = {
    show: function() {
        ...
        const container = document.getElementById('temp-legend-container');
    ...
    updateTime: async function(timeStr = undefined) {
        ...
        updateTempLegend(temperatureData);
    }
};
